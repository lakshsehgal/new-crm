...
export default function OppActivityFeed({
  opportunityId,
  leadId,
  leadName,
  activities,
  ...
  const filtered = items.filter((a) => {
    ...
    if (tab === "conversations")
      return a.kind === "email" || a.type === "CALL" || a.type === "MEETING";
    return true;
  });
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-3">
        {filtered.length === 0 ? (
          ...
          filtered.map((item) =>
            item.kind === "email" ? <EmailRow key={"e" + item.id} email={item} /> : <ActivityRow key={"a" + item.id} activity={item} />,
          )
        )}
      </div>
    </div>
  ...
  return (
    <div className="flex gap-3">
      ...
      <div className="flex-1 min-w-0">
        ...
        {a.body && <p className="text-[13px] text-muted mt-1 whitespace-pre-wrap">{a.body}</p>}
      </div>
    </div>
  );
}
...
function mergeTimeline(activities: Activity[], emails: Email[]): TimelineItem[] {
